...
func TestReElection2A(t *testing.T) {
	...
	t.Logf("%d is disconnected", leader1)
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	cfg.connect(leader1)
	t.Logf("%d is rejoining", leader1)
	...
	t.Logf("%d is disconnected", leader2)
	t.Logf("%d is disconnected", (leader2+1)%servers)
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()

	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	t.Logf("%d is rejoining", (leader2+1)%servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	t.Logf("%d is rejoining", leader2)
	cfg.checkOneLeader()
