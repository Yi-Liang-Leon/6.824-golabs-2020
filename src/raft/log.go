...
const (
	...
	NORMAL_COLOR                 = "\x1b[0m"
	LOG_LEVEL       logrus.Level = logrus.DebugLevel
)
...
func (rf *Raft) DebugLogNoLock(format string, args ...interface{}) {
	...
	logrus.Debugf("%s%d(%d)%s:%s", color_scheme, rf.me, rf.currentTerm, NORMAL_COLOR, fmt.Sprintf(format, args...))
